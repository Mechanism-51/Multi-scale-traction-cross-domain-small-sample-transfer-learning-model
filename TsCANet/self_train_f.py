...
def train_f(model, num_epochs, target_fs, target_d1, lr_set, loss_fn, critizen, path_r, augm):
    num_epochs = 50
    print(f'fine_turn')
    path = os.path.join(path_r,'train_f')
    ...
    all_y_true = []
    all_y_pred = []
    model.load_state_dict(torch.load(os.path.join(os.path.join(path_r, 'train_adapt'), 'model_adapt_20.pt')))
    model.to(DEVICE)
    ...
    for epoch in range(num_epochs):
        ...
        model.cla_f.requires_grad_(True)
        params = [
                  {'params': model.cla_f.parameters(), 'lr': lr_set*0.05}
                  ]
        optimizer = torch.optim.SGD(params,lr=lr_set,weight_decay=0.0001)
        start_time = time.time()
        for x_batch, y_batch in target_fs:
            optimizer.zero_grad()
            x_batch = x_batch[:, :3, :, :]
            x_batch = x_batch.to(DEVICE)
            y_batch = y_batch.to(DEVICE)
            _, _, _, y = model(x_batch,x_batch,x_batch)
        ...
        loss_hist_train[epoch] = loss_hist_train[epoch]/len(target_fs.dataset)
        accuracy_hist_train[epoch] = accuracy_hist_train[epoch]/len(target_fs.dataset)
        if epoch>num_epochs-2:
            torch.save(model.state_dict(), os.path.join(path, 'model_f_{}.pt'.format(epoch+1)))
