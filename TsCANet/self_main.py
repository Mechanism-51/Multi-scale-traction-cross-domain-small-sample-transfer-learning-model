...
from self_model import Res10_model
model = Res10_model(3)




from self_loss_function import PairedCosineLoss
loss_fn = nn.CrossEntropyLoss()
criterion = PairedCosineLoss()
...
hist, y_true, y_pred, path = train_pre(model, para_model['epochs'], source_ds, target_ds, para_model['lr'], loss_fn, para_result['output_file']) #, test_d2_for_CNN
...
def amplitude_scale(spec, scale_range=(0.8, 1.2)):
    ...
    return spec * scale.view(spec.shape[0], 1, 1, 1)
#3. 高斯噪声注入（模拟背景噪声）
...
from self_train_adapt import train_adapt
hist, y_true, y_pred, path = train_adapt(model, para_model['epochs'], source_ds, target_ds, para_model['lr'], loss_fn, criterion, para_result['output_file'], augment_time_frequency) #, test_d2_for_CNN
save_result_fig_and_Excel(path,hist)
conf_mat, f1_score = save_confusion_matrix(y_true, y_pred, path)
print(f'Macro F1 Score: {f1_score:.4f}')
print(f'end')
...
from self_train_f import train_f
hist, y_true, y_pred, path = train_f(model, para_model['epochs'], fs_ds, target_ds, para_model['lr'], loss_fn, criterion, para_result['output_file'], augment_time_frequency) #, test_d2_for_CNN
save_result_fig_and_Excel(path,hist)
conf_mat, f1_score = save_confusion_matrix(y_true, y_pred, path)
print(f'Macro F1 Score: {f1_score:.4f}')
print(f'end')
