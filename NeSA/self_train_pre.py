...
def train_pre(model, num_epochs, source_d1, target_d1, target_fs_d1, lr_set,loss_fn, loss_rec, path_r):
    num_epochs = 100
    lr_set = 0.001
    print(f'train_pre......')
    ...
    for epoch in range(num_epochs):
        model.train()
        optimizer = torch.optim.SGD([{"params": model.parameters()}],lr=lr_set,weight_decay=0.001)
        ...
        for x_batch, y_batch in source_d1:
            ...
            x1, y = model(x_batch)
            _, y2 = model(x1)
            loss = loss_fn(y,y_batch)+loss_fn(y2,y_batch)+loss_rec(x1,x_batch)
            loss.backward()
            optimizer.step()
