...
def train_f1(model, num_epochs, source_d1, target_d1, target_fs_d1, lr_set,loss_fn, loss_rec, path_r):
    num_epochs = 30
    lr_set = 0.001
    print(f'train_f1......')
    path = os.path.join(path_r,'train_f1')
    ...
    model.to(DEVICE)
    model.load_state_dict(torch.load(os.path.join(os.path.join(path_r,'train_pre'), 'model_pre_100.pt')))
    for epoch in range(num_epochs):
        model.train()
        optimizer = torch.optim.SGD([{"params": model.parameters()}],lr=lr_set)
        ...
        if epoch>num_epochs-2:
            torch.save(model.state_dict(), os.path.join(path, 'model_f1_{}.pt'.format(epoch+1)))
