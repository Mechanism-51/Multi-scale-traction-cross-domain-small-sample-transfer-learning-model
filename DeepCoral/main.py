...
def main():
    ...
    model = DeepCORAL(num_classes=NUM_CLASSES)
    if CUDA:
        model = model.cuda()
        print("Using CUDA...")
    ...
    for epoch in trange(EPOCHS):
        ...
        optimizer = torch.optim.SGD([
            ...
            {"params": model.fc8.parameters(), "lr": cur_lr}
        ], lr=cur_lr / 10, momentum=MOMENTUM, weight_decay=L2_DECAY)
        ...
        print("[EPOCH] {}: Classification loss: {:.6f}, CORAL loss: {:.6f}, Total loss: {:.6f}".format(
            epoch + 1,
            sum(r['classification_loss'] / r['total_steps'] for r in result_train),
            sum(r['coral_loss'] / r['total_steps'] for r in result_train),
        ...
        print("[Test Source Confusion Matrix]")
        print(test_source['confusion_matrix'])
        ...
        print("[Test Target Confusion Matrix]")
        print(test_target['confusion_matrix'])
    ...
    if ADAPT_DOMAIN:
        ...
        save_model(model, os.path.join(log_dir, 'adaptation_checkpoint.tar'))
    else:
        ...
        save_model(model, os.path.join(log_dir, 'no_adaptation_checkpoint.tar'))
