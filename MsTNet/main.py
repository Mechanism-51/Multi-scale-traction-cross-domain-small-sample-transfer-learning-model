...
para_dataload = {
    ...
    'sample_number': 1000,
    'sele_target' : 5,
    'trans_h': 256, 'trans_w': 256, 'b_size': 64*2,
}
...
para_result = {
    'output_file': 'T13_3_cwt_{}_samle_{}_target_{}_shuf_{}_C_{}_MMD_{}_classier_{}'.format(para_dataload['cwt'],para_dataload['sample_number'],para_dataload['sele_target'],para_model['shuffer'],para_model['chanels'],para_model['Q_MMD'],para_model['num_classier'])
 } 
...
target_few_shot_spe_list = random.sample(spe_list[t_i[0]][:(n)], t_n) + random.sample(spe_list[t_i[1]][:(n)], t_n) + \
                           random.sample(spe_list[t_i[2]][:(n)], t_n) + random.sample(spe_list[t_i[3]][:(n)], t_n)
print(f'source_spe_list:', len(source_spe_list ))
...
#target_spe_list = target_spe_list + target_spe_list
print(f'target_spe_list×2:',len(target_spe_list))
print(f'target_few_shot_spe_list:',len(target_few_shot_spe_list))

target_few_shot_spe_list = [item for _ in range(int(n*2/t_n)) for item in target_few_shot_spe_list]
print(f'target_few_shot_spe_list×n:',len(target_few_shot_spe_list))
...
s_domain_label = torch.zeros(para_dataload['sample_number']*8,dtype=torch.long)
t_domain_label = torch.ones(para_dataload['sample_number']*8,dtype=torch.long)
source_ds = get_dataloader(source_spe_list, s_domain_label, transform, bs)
target_ds = get_dataloader(target_spe_list, t_domain_label, transform, bs)
fs_ds = get_dataloader(target_few_shot_spe_list, t_domain_label, transform, bs)
...
#加载模型定义训练参数
if para_model['shuffer']:
    from self_model_shuffer_NiN import Model
    #print(f'model_mmf_shuffer_esmble')
else:
    from self_model import Model
    #print(f'model_mmf_esmble')

if int(para_dataload['cwt']):
    model = Model(3,para_model['chanels'],1,para_model['num_classier'])
else:
    model = Model(4,para_model['chanels'],1,para_model['num_classier'])


# 损失函数和优化器
from self_loss_function import MMDLoss
from self_loss_function import DDC_loss
from self_loss_function import loss_margin
loss_fn = loss_margin
...
# 开始训练
from self_train import train
hist, y_true, y_pred = train(model, para_model['epochs'], source_ds, target_ds, fs_ds, para_model['lr'], loss_fn, loss_fn1,  critizen, DDC_loss, para_model['Q_MMD'], para_result['output_file']) #, test_d2_for_CNN
#hist = train(model, para_model['epochs'], source_ds, target_ds, fs_ds, optimizer, loss_fn, DDC_loss, para_model['Q_MMD']) #, test_d2_for_CNN
...
save_result_fig_and_Excel(para_result['output_file'],hist)
conf_mat, f1_score = save_confusion_matrix(y_true, y_pred, para_result['output_file'])
