...
class Multiclassier(nn.Module):
	def __init__(self, channels, drop_p):
		super().__init__()
		self.gap = nn.AdaptiveAvgPool2d((1, 1))
		self.flatten = nn.Flatten()
		self.relu = nn.ReLU(inplace=True)
		self.class_X15 = nn.Linear(channels, 4)
		self.class_X25 = nn.Linear(channels * 2, 4)
		self.class_X35 = nn.Linear(channels * 4, 4)
		self.drop_c1 = nn.Dropout(p=drop_p)
		self.drop_c2 = nn.Dropout(p=drop_p)
		self.drop_c3 = nn.Dropout(p=drop_p)
		self.drop_d1 = nn.Dropout(p=drop_p)
		self.drop_d2 = nn.Dropout(p=drop_p)
		self.drop_d3 = nn.Dropout(p=drop_p)
	def forward(self, X15, X25, X35):
		X15_1 = self.gap(X15)
		X15 = self.relu(self.flatten(X15_1))
		X15 = self.drop_c1(X15)
		Y15 = self.class_X15(X15)
		X25_1 = self.gap(X25)
		X25 = self.relu(self.flatten(X25_1))
		X25 = self.drop_c2(X25)
		Y25 = self.class_X25(X25)
		X35_1 = self.gap(X35)
		X35 = self.relu(self.flatten(X35_1))
		X35 = self.drop_c3(X35)
		Y35 = self.class_X35(X35)
		return Y15, Y25, Y35, X15, X25, X35
...
class Model(nn.Module):
	def __init__(self, inputchannels, channels, m, num_classier):
		...
		self.t_multiclassier = Multiclassier(channels, 0.5)
		self.f_multiclassier = Multiclassier(channels, 0.5)
		self.gap = nn.AdaptiveAvgPool2d((1, 1))
		self.conv2 = nn.Conv2d(channels*2, 4*num_classier, 3, 1, 1)
		self.flatten = nn.Flatten()
	...
	def forward(self, X):
		X = self.conv1(X)
		X = self.maxpool(X)
		Xt, Xt_15, Xt_25, Xt_35 = self.mmf_t(X)
		Xf, Xf_15, Xf_25, Xf_35 = self.mmf_f(X)
		T15, T25, T35, Xt_15, Xt_25, Xt_35 = self.t_multiclassier(Xt_15, Xt_25, Xt_35)
		F15, F25, F35, Xf_15, Xf_25, Xf_35 = self.f_multiclassier(Xf_15, Xf_25, Xf_35)
		X = torch.cat((Xt,Xf), dim=1)
		Y = self.conv2(self.dropout2(X))
		Y = self.flatten(self.gap(Y))



		return Y, T15, T25, T35, F15, F25, F35, Xt_15, Xt_25, Xt_35, Xf_15, Xf_25, Xf_35  #, D, Dt_15, Dt_25, Dt_35, Df_15, Df_25, Df_35  #最后一层集成分类器的结果，多尺度分类器的结果*6，最后一层分类器的输入特征，多尺度分类器的输入特征*6